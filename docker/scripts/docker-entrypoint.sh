...
then
  ...
  echo "kafka-manager.broker-view-thread-pool-size=$(( $((KAFKA_NUMBERS_OF_BROKERS)) * 3))" >> /kafka-manager-${KM_VERSION}/conf/application.conf
  echo "kafka-manager.broker-view-max-queue-size=$(( $((KAFKA_TOTAL_PARTITIONS_NUMBER)) * $((KAFKA_NUMBERS_OF_BROKERS))))" >> /kafka-manager-${KM_VERSION}/conf/application.conf
  echo "kafka-manager.broker-view-update-seconds=$(( $((KAFKA_TOTAL_PARTITIONS_NUMBER)) * $((KAFKA_NUMBERS_OF_BROKERS)) / $((10)) * $((KAFKA_NUMBERS_OF_BROKERS)) ))" >> /kafka-manager-${KM_VERSION}/conf/application.conf
